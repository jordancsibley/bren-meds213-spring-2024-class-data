...
data <- read.csv("week9/southpark-sdc.csv")
```
...
for sdcMicro you need to convert variables to factor and need to convert numeric data
```{r}
fname = "week9/southpark-sdc.csv"
file <- read.csv(fname)
...
sdcInitial <- createSdcObj(dat=file,
                       ...
                       strataVar=NULL,
                       pramVars=NULL,
                       excludeVars=c("dob", "name", "ssn"), #For now, we won't include stu_id; we'll revisit this decision shortly.
                       seed=0,
...
Keeping it so we have a unique identifier for each person, but then we will be replacing this number and then removing student id
...
# get the risk of re identifiication of the whole dataset 
sdcInitial@risk$global$risk

# get the risk of re re identifying an indivdual in the data 
